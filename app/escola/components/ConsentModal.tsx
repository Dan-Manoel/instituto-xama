...
type View = "form" | "termo";

export default function ConsentModal({
  open,
  onClose,
  checkoutUrl,
  titulo = "Confirmação e Consentimento",
}: ConsentModalProps) {
  ...
  // ESC fecha (apenas quando estiver no form; no termo, ESC também fecha — se quiser diferente, eu ajusto)
  useEffect(() => {
    if (!open) return;
  ...
  // volta pro topo ao trocar de "tela"
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        <div className="px-5 md:px-8 pt-6 md:pt-7 pb-3">
          <h2 className="text-2xl md:text-3xl font-semibold text-[var(--texto)]">
            {view === "termo" ? "Termo de Participação e Responsabilidade" : titulo}
          </h2>
          ...
          {view === "form" ? (
            ...
            <p className="mt-2 text-sm md:text-base text-[var(--texto-fraco)]">
              Leia com atenção. Você pode voltar e concluir em seguida.
            </p>
          )}
        </div>

        {/* conteúdo rolável */}
        <div ref={scrollRef} className="px-5 md:px-8 pb-5 overflow-y-auto flex-1">
          {view === "form" ? (
            <>
              {/* frase curta */}
              <div className="rounded-2xl bg-black/5 border border-black/10 p-4 md:p-5 text-[var(--texto)]">
                <p className="leading-relaxed">
                  Parabéns, você está a um passo do caminho do conhecimento.
                </p>
              </div>
              ...
              <div className="mt-4 flex items-start gap-3">
                ...
                <label htmlFor="consent" className="text-sm text-[var(--texto)] leading-relaxed">
                  ...
                  >
                    Termo de Participação e Responsabilidade
                  </button>
            ...
            <>
              {TermoContent}
              {/* REMOVIDO: botão Voltar dentro da área rolável */}
            </>
          )}
        </div>
        ...
        <div className="px-5 md:px-8 py-4 bg-[rgba(254,250,224,0.96)] border-t border-black/10">
          <div className="flex flex-col md:flex-row gap-3 md:gap-4 md:justify-end">
            {view === "form" ? (
              ...
              <>
                {/* NO TERMO: apenas VOLTAR no lugar do Cancelar */}
                <button
                  type="button"
